    ...
    public void loop() {
        ...
        if (detector.getDuckPosition() == 0) {
            ...
            if (ifLoopCount == 0) {
                ...
                encoderDrive(0.15, 6, 6); // Moves the robot 1.5-ish inches forward at 15% speed
            }
            if (runtime.time() >= 0.500 && ifLoopCount == 1) { // Elapsed time 500 milliseconds
                encoderDrive(0.15, 6, -6); // Turns the robot left at 15% speed
                //ifLoopCount++;
            }
            if (runtime.time() >= 0.250 && ifLoopCount == 2) { // Elapsed time 250
                encoderDrive(0.2, -9, -9); // Moves the robot 7-ish inches backwards at 40% speed
                //ifLoopCount++;
            }
            if (runtime.time() >= 0.25 && ifLoopCount == 3) {
                encoderDrive(0.2, -2.5, 2.5);
            }
            if (runtime.time() >= 0.25 && ifLoopCount == 4) {
                encoderDrive(0.2, -4, -4);
            }
            if (runtime.time() >= 0.300 && ifLoopCount == 5) { // Elapsed time 300
                // Run the carousel spinner
                CSpinner.setPower(0.5);
                encoderDrive(0.035, -1, -1);
                //runtime.reset();
                //ifLoopCount++;
            }
            if (runtime.time() >= 5 && ifLoopCount == 6) {
                CSpinner.setPower(0);
                encoderDrive(0.2, 1, 1);
                //ifLoopCount++;
            }
            if (runtime.time() >= 0.15 && ifLoopCount == 7) { // Turns towards depot
                encoderDrive(0.15, -3, 3);
            }
            if (runtime.time() >= 0.25 && ifLoopCount == 8) {
                encoderDrive(0.2, 2.5, 2.5);
            }
            /*
            if (runtime.time() >= 0.250 && ifLoopCount == 9) {
                Err.setPowerCRServo(intakeLeft, 1.0);
                Err.setPowerCRServo(intakeRight, -1.0);
                ifLoopCount++;
            }
            if (runtime.time() >= 1.5 && ifLoopCount == 10) {
